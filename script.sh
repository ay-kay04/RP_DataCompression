...
if [ $tool = "zip" ]
 ...
 then
	...
	echo "Choose between ultra mode(high CR), fast mode(high C/D S) and adapt mode"
	read mode
	
	echo "Compressing using zstd now"
	python3 script.py "tar -I 'zstd --$mode' -cf archive.tar.zst $direct" "compression using zstd, mode $mode"
